...
def starttijden_goedzetten(begintijden):
...
def begin_planning(startlocatie_lijst, eindlocatie_lijst, starttijden, activiteiten, buslijn):
...
def verbruik_matrix(nieuwe_planning):
...
def lengte_rit(nieuwe_planning):
...
def eindtijden_goedzetten(begintijden, duur):
...
def planning_deel2(nieuwe_planning, eindtijden, verbruik, datums, datums_eind, omlopen):
